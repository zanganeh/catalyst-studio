...
- [ ] Git Setup (REQUIRED FIRST TASK)
  ...
  - [ ] Create feature branch: `git checkout -b feature/8-8-url-resolution-routing`

- [ ] Implement URL Resolution Service (AC: 1)
  - [ ] Create `lib/services/url-resolution/url-resolver.ts`
  - [ ] Implement `resolveUrl(path: string)` method using site_structure.full_path
  - [ ] Add trailing slash normalization logic
  - [ ] Implement case-insensitive matching option
  - [ ] Add caching layer for frequently accessed URLs
  - [ ] Write unit tests for URL resolution logic

- [ ] Create Next.js Middleware (AC: 2)
  - [ ] Create new `middleware.ts` in project root (verified: does not exist)
  - [ ] Implement URL resolution middleware function
  - [ ] Integrate with UnifiedPageService for page data retrieval
  - [ ] Pass resolved page context to route handlers
  - [ ] Handle static assets and API routes bypass
  - [ ] Add performance logging for resolution times

- [ ] Implement 404 Handling (AC: 3)
  - [ ] Create custom 404 page at `app/404.tsx`
  - [ ] Implement 404 detection in middleware
  - [ ] Add logging service for 404 tracking
  - [ ] Differentiate between hard 404s (no page) and soft 404s (unpublished)
  - [ ] Create 404 response with suggestions for similar pages
  - [ ] Write tests for 404 scenarios

- [ ] Build Redirect System (AC: 4)
  - [ ] Add redirects table to Prisma schema in `prisma/schema.prisma`
  - [ ] Reset database with new schema: `npm run prisma:reset`
  - [ ] Update seed data to include sample redirects if needed
  - [ ] Implement `RedirectService` in `lib/services/redirect-service.ts`
  - [ ] Add redirect detection in middleware before 404 handling
  - [ ] Implement redirect chain resolution (max 3 hops)
  - [ ] Create admin API endpoints for redirect management
  - [ ] Add redirect validation to prevent loops
  - [ ] Write integration tests for redirect scenarios

- [ ] Enhance Resolution API Endpoint (AC: 5)
  - [ ] Update existing GET `/api/pages/resolve/route.ts` endpoint
  - [ ] Continue using pageOrchestrator.resolveUrl() for URL resolution (UnifiedPageService is for content creation/updates only)
  - [ ] Add support for query parameters: `?include=content,structure,children`
  - [ ] Implement proper error handling with StandardResponse format
  - [ ] Add request validation and path sanitization
  - [ ] Document API endpoint enhancements in OpenAPI spec
  - [ ] Write comprehensive API integration tests

- [ ] Performance Optimization
  - [ ] Add database indexes if not present: `idx_full_path`, `idx_website_id`
  - [ ] Implement in-memory caching for hot paths
  - [ ] Add cache invalidation on page updates
  - [ ] Benchmark resolution performance (target < 10ms)

- [ ] Integration Testing
  - [ ] Test URL resolution with PageOrchestrator
  - [ ] Test with UnifiedPageService
  - [ ] Verify redirect chains work correctly
  - [ ] Test 404 handling for various scenarios
  - [ ] Verify middleware doesn't break existing routes
  - [ ] Load test with 100+ concurrent resolutions
