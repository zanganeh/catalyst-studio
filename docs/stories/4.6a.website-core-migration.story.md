...
6. Create migration script for existing website metadata
...
- [ ] Create Migration Script (AC: 6)
  - [ ] Create `/scripts/migrate-website-metadata.ts`:
    - Detect existing website data in localStorage
    - Read website metadata and settings
    - Transform to database format
    - Create via API
    - Verify migration success
    - Mark as migrated to prevent re-runs
  - [ ] Add script to package.json: `"migrate:websites": "tsx scripts/migrate-website-metadata.ts"`
- [ ] Testing and Validation (AC: 2, 3, 4, 5)
...
- Test migration script with edge cases
...
3. **Migration Safety**: Check for already-migrated flag to prevent duplicates
4. **Backward Compatibility**: Keep localStorage read as fallback during transition
